echo "--- Creating Conda Environment 'awd-lstm' ---"
conda create -n awd-lstm python=3.8 -y
...
pip install comet_ml debugpy wandb 
...
git config --global user.email "eergun@ucsd.edu"
git config --global user.name "Esen Ergun"
